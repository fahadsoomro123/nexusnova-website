...
import json, re
import specialists_v6 as experts
...
def make_plan(core,base,model,system_prompt,user,mode,level,context=''):
    if level=='fast':return {'steps':[],'success':[],'risks':[],'summary':'Fast task','specialists':[],'council':[]}
    council=experts.council(core,base,model,system_prompt,user,mode,level,context)
    council_text=experts.planner_context(council)
    planner='''You are NOVA AI POWER V6 Ultimate Planner. You receive evidence-backed project context plus an independent specialist council. Create one unified execution plan for a tool-using worker.
Return ONLY JSON: {"summary":"...","steps":["..."],"success":["..."],"risks":["..."]}.
Rules: do not pretend work is done; reconcile specialist advice instead of blindly copying it; use supplied project context; inspect before editing; for code changes require non-main branch, minimal reversible changes, checks/build, diff/evidence. For Builder mode include scaffold/inspect/implementation/build/export only when relevant. Keep 4-12 concrete steps and 2-7 success criteria.'''
    msg=f'MODE: {mode}\nLEVEL: {level}\nOWNER REQUEST:\n{user}\n\nPROJECT CONTEXT:\n{context[:18000] or "(none yet)"}\n\n{council_text or "SPECIALIST COUNCIL: unavailable"}'
    try:
        raw=_call(core,base,model,[{'role':'system','content':system_prompt+'\n\n'+planner},{'role':'user','content':msg}],0.06)
        data=_extract_json(raw,{})
        return {
          'summary':str(data.get('summary') or '')[:1200],
          'steps':[str(x)[:900] for x in (data.get('steps') or [])[:12]],
          'success':[str(x)[:700] for x in (data.get('success') or [])[:7]],
          'risks':[str(x)[:700] for x in (data.get('risks') or [])[:7]],
          'specialists':council.get('roles',[]),
          'council':council.get('advice',[])[:3]
        }
    except Exception:return {'steps':[],'success':[],'risks':[],'summary':'','specialists':council.get('roles',[]),'council':council.get('advice',[])[:3]}


def worker_plan_text(plan):
    if not plan or not plan.get('steps'):return ''
    lines=['V6 ULTIMATE EXECUTION PLAN:']
    if plan.get('specialists'):lines.append('Specialist Council: '+', '.join(plan['specialists']))
    lines += [f'{i+1}. {x}' for i,x in enumerate(plan['steps'])]
    ...
    if plan.get('council'):
        lines.append('SPECIALIST CHECKS:')
        for row in plan['council']:
            for x in row.get('checks',[])[:3]:lines.append(f"- {row.get('title',row.get('role','Expert'))}: {x}")
    return '\n'.join(lines)[:15000]
...
def review(core,base,model,system_prompt,user,mode,level,draft,evidence,plan):
    ...
    reviewer='''You are NOVA AI POWER V6 Ultimate Evidence Reviewer. Judge the worker using ONLY owner request, plan/success criteria, specialist council expectations, final draft and structured tool evidence. Be strict about false completion claims, missing inspection, missing build/tests after code edits, unsafe main-branch edits, missing diff/check evidence, stale research, likely regressions, and unverified deploy/live claims. Honest limitations may still pass. Return ONLY JSON: {"pass":true|false,"score":0.0,"issues":["..."],"correction":"...","summary":"..."}. A pass should normally score >=0.72.'''
    payload={'owner_request':user,'mode':mode,'level':level,'plan':plan,'worker_draft':draft[:22000],'evidence':evidence[-90:]}
    try:
        ...
        passed=bool(data.get('pass')) and score>=0.72
        return {'pass':passed,'score':score,'issues':[str(x)[:700] for x in (data.get('issues') or [])[:10]],'correction':str(data.get('correction') or '')[:4000],'summary':str(data.get('summary') or '')[:1200]}
...
def critic(core,base,model,system_prompt,user,mode,level,draft,evidence,review_result):
    ...
    critic_prompt='''You are NOVA AI POWER V6 Ultimate Red-Team Critic, a second independent checker. Try to falsify the worker's success claim. Look for material gaps the reviewer may miss: unmet owner requirement, regression, insecure behavior, broken mobile usability, missing responsiveness/accessibility, insufficient project context, build not actually run, broad rewrite, missing generated artifact, or unsupported claim. Do not nitpick style. Return ONLY JSON: {"pass":true|false,"issues":["..."],"correction":"...","summary":"..."}.'''
    payload={'request':user,'mode':mode,'draft':draft[:18000],'evidence':evidence[-90:],'review':review_result}
    try:
...
def correction_note(plan,review_result,critic_result,attempt):
    ...
    return f'''NOVA AI POWER V6 ULTIMATE RECOVERY PASS {attempt}.
...
def should_auto_verify(evidence):
    mutating={'write_file','scaffold_app','git_commit','git_commit_safe'}
