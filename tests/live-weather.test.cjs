...
test('visitor browser reads cached weather JSON and never calls MET Norway directly',()=>{const client=read('assets/js/live-weather.js');assert.match(client,/assets\/data\/live-weather\.json/);assert.doesNotMatch(client,/api\.met\.no/)});

test('weather page labels model forecast honestly and fixed worldwide scope',()=>{const page=read('weather-live.html');assert.match(page,/not observed station readings/i);assert.match(page,/Data from MET Norway/i);assert.match(page,/Model updated:/i);assert.match(page,/fixed, cached worldwide-city set/i);assert.match(page,/arbitrary global search/i)});

test('Pakistan and worldwide weather coordinates follow four-decimal cache guidance',async()=>{const moduleUrl=pathToFileURL(path.join(root,'.github/scripts/live_weather_core.mjs')).href;const {PAKISTAN_CITIES,WORLD_CITIES,ALL_CITIES}=await import(moduleUrl);assert.equal(PAKISTAN_CITIES.length,6);assert.equal(WORLD_CITIES.length,6);assert.equal(ALL_CITIES.length,12);for(const city of ALL_CITIES){assert.equal(Number(city.lat.toFixed(4)),city.lat);assert.equal(Number(city.lon.toFixed(4)),city.lon);assert.ok(city.country)}});
