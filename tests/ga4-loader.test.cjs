...
test('GA4 loads automatically with the existing measurement ID and one page_view config', () => {
  ...
  assert.doesNotMatch(wrapper, /window\\.__nexusnovaConsentReady=true/);
...
test('site shell retains the privacy-choice UI and Consent Mode state', () => {
  ...
  assert.match(text, /data-nexusnova-consent/);
