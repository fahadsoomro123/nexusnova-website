...
def complete(label,a):
 ...
 return True
def call(prompt):
 req=urllib.request.Request(BASE+'/api/nova',method='POST',data=json.dumps({'message':prompt,'focus':'auto'}).encode(),headers={'Origin':'https://nexusnovatools.com','Accept':'application/json','Content-Type':'application/json','User-Agent':'NexusNova-Reliability/3.0'}); started=time.time()
 try:
...
def evaluate_api(label,status,body,lat):
 a=str(body.get('answer','')).strip(); numeric=bool(re.fullmatch(r'[\d\s,.%+\-*/()×÷:$]+',a)); return {'http':status,'mode':body.get('mode'),'provider':body.get('provider'),'providerAttempts':body.get('providerAttempts'),'providerFailure':body.get('providerFailure'),'requestId':body.get('requestId'),'responseLength':len(a),'complete':complete(label,a),'naturalLanguage':natural(a),'unexplainedTruncation':bool(a.endswith('...') or a.endswith('…')),'numericOnly':numeric,'answerPrefix':a[:3000],'latencySec':round(lat,2),'success':bool(status==200 and body.get('mode')=='answer' and body.get('provider')=='gemini' and complete(label,a) and not numeric)}
results={'api':{},'browser':{}}
...
browser_errors=[]; viewports=[('mobile390',390,844,'Mozilla/5.0 (Linux; Android 11; Mobile) AppleWebKit/537.36 Chrome/151.0.0.0 Mobile Safari/537.36'),('mobile412',412,915,'Mozilla/5.0 (Linux; Android 11; Mobile) AppleWebKit/537.36 Chrome/151.0.0.0 Mobile Safari/537.36'),('desktop1280',1280,800,None)]
with sync_playwright() as p:
 ...
 for name,w,h,ua in viewports:
  ...
  script_src=page.locator('script[src*="assets/nova/nova-ui.js"]').get_attribute('src') or ''; old_count=page.locator('script[src*="assets/js/nova-intelligence.js"]').count(); print(name.upper(),'NOVA_SCRIPT=',script_src,'OLD_SCRIPT_COUNT=',old_count)
  ...
  for label,prompt in TESTS.items():
   ...
   for rep in range(1,reps+1):
    try:
     ...
     with page.expect_response(lambda r:r.url==BASE+'/api/nova' and r.request.method=='POST',timeout=75000) as ev:
      page.locator('#niBuild').click(force=True)
     processing=page.locator('#niState').inner_text(timeout=2000).strip(); response=ev.value; status=response.status; body=response.json(); a=str(body.get('answer','')).strip(); rendered=page.locator('#niResult').inner_text().strip(); state=page.locator('#niState').inner_text().strip(); metrics=page.evaluate('() => ({scrollWidth:document.documentElement.scrollWidth,clientWidth:document.documentElement.clientWidth})')
     same=bool(a and a in rendered); numeric=bool(re.fullmatch(r'[\d\s,.%+\-*/()×÷:$]+',a)); fallback=('No fake answer was generated' in rendered or 'connection unavailable' in rendered.lower()); tags={'headings':page.locator('#niResult h1,#niResult h2,#niResult h3').count(),'uls':page.locator('#niResult ul').count(),'ols':page.locator('#niResult ol').count(),'tables':page.locator('#niResult table').count(),'code':page.locator('#niResult pre').count()}; expected_structure=(label!='TEST 5' or tags['tables']>0)
     item={'http':status,'mode':body.get('mode'),'provider':body.get('provider'),'providerAttempts':body.get('providerAttempts'),'providerFailure':body.get('providerFailure'),'requestId':body.get('requestId'),'responseLength':len(a),'complete':complete(label,a),'naturalLanguage':natural(a),'unexplainedTruncation':bool(a.endswith('...') or a.endswith('…')),'numericOnly':numeric,'uiDisplayedFullAnswer':same,'fallbackUi':fallback,'processingState':processing,'finalState':state,'structure':tags,'expectedStructure':expected_structure,'consoleErrors':errors[-10:],'viewport':metrics,'answerPrefix':a[:3000]}; item['success']=bool(status==200 and body.get('mode')=='answer' and body.get('provider')=='gemini' and item['complete'] and same and not fallback and not numeric and processing in ('SUBMITTING','PROCESSING','STILL WORKING') and state not in ('READY','ERROR — RETRY') and metrics['scrollWidth']<=metrics['clientWidth']+6 and expected_structure)
  ...
  ctx.close()
 browser.close()
Path('nova-reliability-results.json').write_text(json.dumps(results,indent=2,ensure_ascii=False)+'\n',encoding='utf-8'); case_pass={label:all(x['success'] for x in results['api'][label]) and all(x['success'] for v in results['browser'].values() for x in v[label]) for label in TESTS}; passed=sum(case_pass.values()); t3api=sum(x['success'] for x in results['api']['TEST 3']); t10api=sum(x['success'] for x in results['api']['TEST 10']); summary={'casePass':case_pass,'passedCases':passed,'totalCases':10,'caseSuccessRate':passed/10,'apiAttempts':sum(len(v) for v in results['api'].values()),'apiFailures':sum(not x['success'] for v in results['api'].values() for x in v),'test3ApiSuccess':f'{t3api}/3','test10ApiSuccess':f'{t10api}/3','browserConsoleErrors':browser_errors[:100]}; print('FINAL_BATTERY_SUMMARY=',json.dumps(summary,indent=2,ensure_ascii=False)); Path('nova-reliability-summary.json').write_text(json.dumps(summary,indent=2,ensure_ascii=False)+'\n',encoding='utf-8');
