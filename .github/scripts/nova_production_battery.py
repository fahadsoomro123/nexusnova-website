...
BASE='https://nexusnova-telegram-bot.fahadsoomro123.workers.dev'; PAGE='https://nexusnovatools.com/nova-intelligence.html?qa='+os.environ['GITHUB_SHA_VALUE']; RATE_GAP=4.5; last_request=0.0
...
def pace():
 global last_request
 wait=RATE_GAP-(time.time()-last_request)
...
def complete(label,a):
 ...
 if label=='TEST 5': return all(t in x for t in ['day','task','duration','goal']) and any(str(n) in x for n in range(1,8))
 ...
 return True
def call(prompt):
 pace(); req=urllib.request.Request(BASE+'/api/nova',method='POST',data=json.dumps({'message':prompt,'focus':'auto'}).encode(),headers={'Origin':'https://nexusnovatools.com','Accept':'application/json','Content-Type':'application/json','User-Agent':'NexusNova-Reliability/3.0'}); started=time.time()
 try:
...
with sync_playwright() as p:
 ...
 for name,w,h,ua in viewports:
  ...
  for label,prompt in TESTS.items():
   ...
   for rep in range(1,reps+1):
    try:
     pace(); page.locator('#niPrompt').fill(prompt)
