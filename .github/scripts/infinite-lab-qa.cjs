...
(async()=>{
 ...
 await test('103 renderer telemetry stays finite',async()=>{const r=await desktop.evaluate(()=>window.__nnRuntime||{});for(const k of ['fps','dpr','quality']){if(k in r)assert(Number.isFinite(Number(r[k])),k+' telemetry is not finite') }});
 ...
 await test('107 second dive reaches Milky Way scene',async()=>{await desktop.waitForTimeout(900);await desktop.evaluate(()=>{for(let i=0;i<3;i++)window.NexusNovaInfiniteLab.zoomBy(.34);window.NexusNovaInfiniteLab.dive();});await desktop.waitForFunction(()=>window.NexusNovaInfiniteLab.getState().depth===2,null,{timeout:3000});await desktop.waitForTimeout(220);assert((await desktop.evaluate(()=>window.__nnRuntime?.scene))==='milkyway','second dive did not enter Milky Way scene')});
