...
(async()=>{
 ...
 await test('107 second dive reaches Milky Way scene',async()=>{await desktop.evaluate(async()=>{for(let i=0;i<3;i++)window.NexusNovaInfiniteLab.zoomBy(.34);for(let i=0;i<18&&window.NexusNovaInfiniteLab.getState().depth<2;i++){window.NexusNovaInfiniteLab.dive();await new Promise(r=>setTimeout(r,100));}});await desktop.waitForFunction(()=>window.NexusNovaInfiniteLab.getState().depth===2,null,{timeout:5000});assert((await desktop.evaluate(()=>window.__nnRuntime?.scene))==='milkyway','second dive did not enter Milky Way scene')});
