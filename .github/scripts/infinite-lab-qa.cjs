...
(async()=>{
 ...
 await test('07 no 2D renderer source',async()=>{const body=await desktop.locator('body').textContent();const src=await desktop.locator('script[src*="infinite-lab.js"]').getAttribute('src');assert(!src.includes('2d'),'2D fallback script path not relevant');assert(!body.includes('2D canvas is the primary'),'misleading renderer copy');});
 ...
 await test('57 Universe Atlas integration link',async()=>{const resp=await desktop.request.get('http://127.0.0.1:4173/labs.html');assert(resp.ok(),'Labs page not served')});
 ...
 await test('59 visible focus style',async()=>{await desktop.locator('#resetBtn').focus();const outline=await desktop.locator('#resetBtn').evaluate(el=>getComputedStyle(el).outlineStyle);assert(outline!=='none','focus style absent')});
 ...
 await test('61 body rejects complete-map claim',async()=>{const t=await desktop.locator('body').innerText();for(const bad of ['complete map of the observable universe','every star in the universe is displayed','every galaxy is loaded','actual telescope surface image'])assert(!t.toLowerCase().includes(bad.toLowerCase()),'deceptive claim found: '+bad)});
 ...
 await test('73 mocked Gaia data normalization',async()=>{await mock.evaluate(()=>window.NexusNovaInfiniteLabCatalog.loadVisibleRegion());await mock.waitForTimeout(300);assert((await mock.evaluate(()=>window.NexusNovaInfiniteLabCatalog.getObjects().some(o=>o.sourceKey==='gaia'))),'Gaia normalization failed')});
