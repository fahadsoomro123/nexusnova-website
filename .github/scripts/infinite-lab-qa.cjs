...
(async()=>{
 ...
 await test('106 dive changes scene from solar to stellar neighborhood',async()=>{await desktop.evaluate(()=>{window.NexusNovaInfiniteLab.reset();for(let i=0;i<3;i++)window.NexusNovaInfiniteLab.zoomBy(.34);window.NexusNovaInfiniteLab.dive();});await desktop.waitForTimeout(180);assert((await desktop.evaluate(()=>window.__nnRuntime?.scene))==='neighborhood','dive did not enter stellar neighborhood scene')});
 await test('107 second dive reaches Milky Way scene',async()=>{await desktop.evaluate(()=>{for(let i=0;i<3;i++)window.NexusNovaInfiniteLab.zoomBy(.34);window.NexusNovaInfiniteLab.dive();});await desktop.waitForTimeout(180);assert((await desktop.evaluate(()=>window.__nnRuntime?.scene))==='milkyway','second dive did not enter Milky Way scene')});
