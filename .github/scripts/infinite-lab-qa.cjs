...
(async()=>{
 ...
 await test('109 WebGL framebuffer contains visible scene pixels',async()=>{const px=await desktop.locator('#universe').evaluate(el=>{const gl=el.getContext('webgl2');const x=Math.max(0,Math.floor(el.width/2)-2),y=Math.max(0,Math.floor(el.height/2)-2),buf=new Uint8Array(5*5*4);gl.readPixels(x,y,5,5,gl.RGBA,gl.UNSIGNED_BYTE,buf);let sum=0,peak=0;for(let i=0;i<buf.length;i+=4){sum+=buf[i]+buf[i+1]+buf[i+2];peak=Math.max(peak,buf[i]+buf[i+1]+buf[i+2]);}return{sum,peak}});assert(px.peak>12&&px.sum>180,'main WebGL framebuffer is effectively blank: '+JSON.stringify(px))});
 ...
 await test('107 second dive reaches Milky Way scene',async()=>{await desktop.waitForFunction(()=>window.NexusNovaInfiniteLab.getState().transition===0,{timeout:2500});await desktop.evaluate(()=>{for(let i=0;i<3;i++)window.NexusNovaInfiniteLab.zoomBy(.34);window.NexusNovaInfiniteLab.dive();});await desktop.waitForFunction(()=>window.NexusNovaInfiniteLab.getState().depth===2,{timeout:2500});assert((await desktop.evaluate(()=>window.__nnRuntime?.scene))==='milkyway','second dive did not enter Milky Way scene')});
