...
case_pass={l: all(x['success'] for x in results['api'][l]) and all(x['success'] for x in results['browser']['mobile'][l]) and all(x['success'] for x in results['browser']['desktop'][l]) for l in TESTS}
passed=sum(case_pass.values()); t3_api=sum(x['success'] for x in results['api']['TEST 3']); t3_browser=sum(x['success'] for x in results['browser']['mobile']['TEST 3'])+sum(x['success'] for x in results['browser']['desktop']['TEST 3'])
summary={'casePass':case_pass,'passedCases':passed,'totalCases':10,'caseSuccessRate':passed/10,'apiAttempts':sum(len(v) for v in results['api'].values()),'apiFailures':sum(not x['success'] for v in results['api'].values() for x in v),'test3ApiSuccess':f'{t3_api}/3','test3BrowserSuccess':f'{t3_browser}/6','browserConsoleErrors':browser_errors[:100]}
...
if passed<9 or t3_api!=3 or t3_browser!=6 or browser_errors: raise SystemExit('PRODUCTION RELIABILITY BATTERY FAILED')
