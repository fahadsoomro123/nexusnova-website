...
export async function searchWeb({ env, query, count = 5 }) {
  ...
  if (config.searchKey) {
    ...
    url.searchParams.set('q', limitedQuery); url.searchParams.set('count', String(safeCount));
    try {
  ...
  if (config.searchUrl) {
    try {
      const base = new URL(config.searchUrl); base.searchParams.set('q', limitedQuery); base.searchParams.set('count', String(safeCount));
...
function buildPrompt(messages, toolCatalog, focus) {
  const safeMessages = messages.slice(-8).map(message => ({ role: message.role === 'assistant' ? 'assistant' : 'user', content: String(message.content || '').slice(0, 6_000) }));
...
function extractGeminiVisibleText(data) {
  return (data?.candidates || []).flatMap(candidate => Array.isArray(candidate?.content?.parts) ? candidate.content.parts : []).filter(part => part && part.thought !== true).map(part => typeof part?.text === 'string' ? part.text : '').filter(Boolean).join('\n').trim().slice(0, DEFAULT_LIMITS.maxOutputChars);
}
function geminiCandidateState(data) {
  const candidate = Array.isArray(data?.candidates) ? data.candidates[0] || {} : {};
  const usage = data?.usageMetadata || {};
  return { truncated: candidate?.finishReason === 'MAX_TOKENS', finishReason: cleanText(candidate?.finishReason, 40), outputTokens: Number(usage?.candidatesTokenCount || 0) || 0, thoughtTokens: Number(usage?.thoughtsTokenCount || 0) || 0 };
}
...
function classifyGeminiResponse(data) {
  const status = cleanText(data?.error?.status, 80); const message = cleanText(data?.error?.message, 180).toLowerCase();
