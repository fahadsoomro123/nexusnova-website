...
export function providerConfig(env) {
  return {
    ...
    geminiFallbackModel: String(env.GEMINI_FALLBACK_MODEL || DEFAULT_GEMINI_FALLBACK_MODEL).trim(),
...
async function callGemini(config,prompt,attempts){
  const primary=await callGeminiModel(config,prompt,attempts,config.geminiModel,[25_000],1);
  if(primary)return primary;
  const lastAttempt=attempts.at(-1);
  const retryablePrimaryFailure=lastAttempt?.provider==='gemini'&&lastAttempt?.model===config.geminiModel&&isRetryableProviderFailure(lastAttempt);
  const fallbackModel=config.geminiFallbackModel;
  if(!fallbackModel||fallbackModel===config.geminiModel||!retryablePrimaryFailure)return null;
  attempts.push({provider:'gemini',model:fallbackModel,ok:false,reason:'fallback-after-primary-failure'});
...
async function callGeminiModel(config,prompt,attempts,model,timeouts,maxAttempts){
  ...
  for(let attempt=0;attempt<maxAttempts;attempt+=1){
    ...
    try{
      ...
      if(!response.ok){
        const responseClass=classifyGeminiResponse(data); const retryAfterMs=parseRetryAfterMs(response.headers.get('Retry-After')); const reason=`http-${response.status}`;
        attempts.push({provider:'gemini',model,ok:false,reason,latencyMs,statusText:cleanText(response.statusText,80),responseClass,retryAfterMs});
        if(![408,429,500,502,503,504].includes(response.status)||attempt===maxAttempts-1)return null;
        await sleep(retryAfterMs>0?Math.min(retryAfterMs,20_000):backoffWithJitter(attempt)); continue;
      }
...
function backoffWithJitter(attempt){const base=[1_500,3_000,6_000][attempt]||6_000;return Math.round(base*(0.75+Math.random()*0.5));}
...
function isRetryableProviderFailure(attempt){return attempt?.reason?.startsWith('http-')&&[408,429,500,502,503,504].includes(Number(attempt.reason.slice(5)));}
