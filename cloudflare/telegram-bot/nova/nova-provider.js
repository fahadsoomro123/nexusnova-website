...
const DEFAULT_GEMINI_FALLBACK_MODELS = ['gemini-3.5-flash-lite', 'gemini-3.8-flash'];
...
export function providerConfig(env) {
  return {
    ...
    geminiFallbackModels: uniqueModels([String(env.GEMINI_FALLBACK_MODEL || '').trim(), ...DEFAULT_GEMINI_FALLBACK_MODELS]),
...
export async function searchWeb({ env, query, count = 5 }) {
  ...
  if (config.searchKey) { const url = new URL('https://api.search.brave.com/res/v1/web/search'); url.searchParams.set('q', limitedQuery); url.searchParams.set('count', String(safeCount)); try { const response = await fetchWithTimeout(url,{headers:{Accept:'application/json','X-Subscription-Token':config.searchKey}},8_000); if(!response.ok)return{ok:false,reason:`search-http-${response.status}`}; const data=await response.json().catch(()=>null); const results=(data?.web?.results||[]).slice(0,safeCount).map(item=>({title:cleanText(item?.title,220),url:safeHttpUrl(item?.url),description:cleanText(item?.description,500),age:cleanText(item?.age,100)})).filter(item=>item.url); return{ok:true,provider:'brave',results}; } catch(error){ return{ok:false,reason:classifyNetworkError(error)}; } }
  if (config.searchUrl) { try { const base=new URL(config.searchUrl); base.searchParams.set('q',limitedQuery); base.searchParams.set('count',String(safeCount)); const response=await fetchWithTimeout(base,{headers:{Accept:'application/json'}},8_000); if(!response.ok)return{ok:false,reason:`search-http-${response.status}`}; const data=await response.json().catch(()=>null); const rows=Array.isArray(data?.results)?data.results:Array.isArray(data?.web?.results)?data.web.results:[]; const results=rows.slice(0,safeCount).map(item=>({title:cleanText(item?.title,220),url:safeHttpUrl(item?.url||item?.link),description:cleanText(item?.description||item?.snippet,500),age:cleanText(item?.age,100)})).filter(item=>item.url); return{ok:true,provider:'configured-search',results}; } catch(error){ return{ok:false,reason:classifyNetworkError(error)}; } }
...
async function callGemini(config,prompt,attempts){
  const models=uniqueModels([config.geminiModel,...config.geminiFallbackModels]);
  for(const model of models){
    const result=await callGeminiModel(config,prompt,attempts,model);
    if(result)return result;
    const last=attempts.at(-1);
    if(!last||last.provider!=='gemini'||!isProviderSwitchableFailure(last))break;
  }
  return null;
}
async function callGeminiModel(config,prompt,attempts,model){
  const url=`https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(model)}:generateContent`;
  const body={contents:[{role:'user',parts:[{text:prompt}]}],generationConfig:{responseMimeType:'application/json',maxOutputTokens:GEMINI_MAX_OUTPUT_TOKENS,thinkingConfig:{thinkingLevel:model.includes('flash-lite')?'minimal':'low'}}};
  const startedAt=Date.now();
  try{
    const response=await fetchWithTimeout(url,{method:'POST',headers:{'Content-Type':'application/json','x-goog-api-key':config.geminiKey},body:JSON.stringify(body)},DEFAULT_LIMITS.timeoutMs);
    const latencyMs=Date.now()-startedAt; const data=await response.json().catch(()=>null);
    if(!response.ok){const responseClass=classifyGeminiResponse(data);const retryAfterMs=parseRetryAfterMs(response.headers.get('Retry-After'));attempts.push({provider:'gemini',model,ok:false,reason:`http-${response.status}`,latencyMs,statusText:cleanText(response.statusText,80),responseClass,retryAfterMs});if(retryAfterMs>0&&isTransientStatus(response.status)){await sleep(Math.min(retryAfterMs,20_000));}return null;}
    const candidateState=geminiCandidateState(data);
    if(candidateState.truncated){attempts.push({provider:'gemini',model,ok:false,reason:'truncated-max-tokens',latencyMs,finishReason:candidateState.finishReason,responseTokens:candidateState.outputTokens,thoughtTokens:candidateState.thoughtTokens});return null;}
    const text=extractGeminiVisibleText(data);const parsed=parseStructuredJson(text);
    if(parsed)return{ok:true,plan:parsed,model};
  ...
  }catch(error){attempts.push({provider:'gemini',model,ok:false,reason:classifyNetworkError(error),latencyMs:Date.now()-startedAt,detail:safeErrorDetail(error)});}
  return null;
}
...
function parseRetryAfterMs(value){const raw=String(value||'').trim();if(!raw)return 0;const seconds=Number(raw);if(Number.isFinite(seconds))return Math.max(0,seconds*1000);const date=Date.parse(raw);return Number.isFinite(date)?Math.max(0,date-Date.now()):0;}
function isTransientStatus(status){return[408,429,500,502,503,504].includes(Number(status));}
function isProviderSwitchableFailure(attempt){return Boolean(attempt&&attempt.provider==='gemini'&&(attempt.reason==='invalid-structured-output'||attempt.reason==='truncated-max-tokens'||attempt.reason==='timeout'||attempt.reason==='network-error'||(attempt.reason||'').startsWith('http-')&&isTransientStatus(Number(attempt.reason.slice(5)))));}
