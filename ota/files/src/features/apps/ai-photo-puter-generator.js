...
export function installPuterImageGenerator(root){
  ...
  async function refreshSession(){try{const session=await getPuterImageSession();if(destroyed)return;status.classList.toggle('is-ok',session.signedIn);statusTitle.textContent=session.signedIn?`Connected${session.user?.username?` · ${session.user.username}`:''}`:'Puter ready';statusCopy.textContent=session.signedIn?usageText(session.usage):'Generate directly. Puter will ask for access if it needs it.';connect.textContent=session.signedIn?'Connected':'Connect Puter';connect.disabled=session.signedIn;generate.disabled=busy}catch(e){if(destroyed)return;statusTitle.textContent='Puter unavailable';statusCopy.textContent=e?.message||'Could not load Puter AI.';generate.disabled=true}}
  ...
  connect.onclick=async()=>{if(busy)return;setError('');connect.disabled=true;connect.textContent='Connecting…';try{await ensurePuterImageSdk();await signInPuterForImages();await refreshSession()}catch(e){connect.disabled=false;connect.textContent='Connect Puter';setError(e?.msg||e?.message||'Puter sign-in did not complete. You can still tap Generate and let Puter request access automatically.')}};
  generate.onclick=async()=>{if(busy)return;const text=prompt.value.trim();if(text.length<3){setError('Write what image you want first.');prompt.focus();return}setError('');busy=true;generate.disabled=true;generate.textContent='Generating…';result.classList.remove('is-on');let beforeUsage=null;try{beforeUsage=(await getPuterImageSession()).usage}catch{}try{const out=await generateAiImage(text,{aspect:q('[data-puter-aspect]').value,style:q('[data-puter-style]').value,mode:q('[data-puter-mode]').value});if(destroyed)return;generated={...out,prompt:text};image.src=out.dataUrl;q('[data-puter-result-title]').textContent=safeName(text).slice(0,64);const usageLabel=perImageUsageText(beforeUsage,out.usage);meta.textContent=`${out.width&&out.height?`${out.width} × ${out.height} · `:''}${out.model||'Puter AI'} · ${usageLabel}`;result.classList.add('is-on');statusCopy.textContent=usageText(out.usage);refreshSession().catch(()=>{})}catch(e){setError(e?.msg||e?.message||'Image generation failed.')}finally{busy=false;generate.textContent='Generate Image';if(!destroyed)generate.disabled=false}};
