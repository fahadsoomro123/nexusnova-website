...
function drawImageElement(ctx,el,x,y,w,h){const image=imageFor(el.src);if(!image){drawPhotoPlaceholder(ctx,el,x,y,w,h);return}ctx.save();roundedRect(ctx,x,y,w,h,(Number(el.radius)||.03)*Math.min(ctx.canvas.width,ctx.canvas.height));ctx.clip();const scale=Math.max(w/image.naturalWidth,h/image.naturalHeight),dw=image.naturalWidth*scale,dh=image.naturalHeight*scale;ctx.drawImage(image,x+(w-dw)/2,y+(h-dh)/2,dw,dh);ctx.restore()}
