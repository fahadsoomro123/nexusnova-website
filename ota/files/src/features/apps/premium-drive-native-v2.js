...
function normalizeRoute(raw) {
  return Array.isArray(raw) ? raw.map(normalizePoint).filter(Boolean).slice(0, 1200) : [];
}
...
async function importCompletedTrip(completed) {
  ...
  const routePoints = normalizeRoute(completed.routePoints || completed.points);
  const startPoint = normalizePoint(completed.startPoint || completed.startCoordinate || completed.start);
  const endPoint = normalizePoint(completed.endPoint || completed.endCoordinate || completed.end);
  const startName = String(completed.startName || completed.startLabel || completed.startLocation || '').trim().slice(0, 160);
  const endName = String(completed.endName || completed.endLabel || completed.endLocation || '').trim().slice(0, 160);
...
export function renderNovaDriveNativeV2() {
  ...
  const showStoredTripMetrics = last => {
    if (!last || snapshot.active === true) return;
    const distance = Math.max(0, Number(last.distanceM) || 0);
    const moving = Math.max(0, Number(last.movingMs) || 0);
    const avg = moving > 0 ? (distance / (moving / 1000)) * 3.6 : Math.max(0, Number(last.avgKmh) || 0);
    if (distanceEl) distanceEl.textContent = distanceText(distance);
    if (averageEl) averageEl.textContent = `${Math.round(avg)} km/h`;
    if (topEl) topEl.textContent = `${Math.round(Math.max(0, Number(last.topKmh) || 0))} km/h`;
  ...
  const paint = detail => {
    ...
    const rawSpeed = Math.max(0, Number(detail.speedKmh) || 0);
    const speed = paused || !active || rawSpeed > 180 ? 0 : rawSpeed;
    const top = Math.max(0, Math.min(180, Number(detail.topKmh) || 0));
