...
function routeMapHtml(a,b){
  ...
  return `<div class="nn-rdv-map"><iframe title="Flight route map" loading="lazy" referrerpolicy="no-referrer" src="${esc(src)}"></iframe><svg viewBox="0 0 100 100" preserveAspectRatio="none" aria-hidden="true"><path d="M12 68 Q50 18 88 32" fill="none" stroke="#68eaff" stroke-width="1.6" stroke-dasharray="3 2"/><circle cx="12" cy="68" r="2.2" fill="#fff"/><circle cx="88" cy="32" r="2.2" fill="#fff"/></svg></div><div class="nn-rdv-map-note">${esc(a.label||'Origin')} → ${esc(b.label||'Destination')}</div>`;
}
function hotelMapHtml(point){
  ...
  return `<div class="nn-rdv-map"><iframe title="Hotel location map" loading="lazy" referrerpolicy="no-referrer" src="${esc(src)}"></iframe></div><div class="nn-rdv-map-note">${esc(point.label||'Verified map location')}<br><a class="nn-rdv-open-map" href="${esc(open)}" target="_blank" rel="noopener noreferrer">OPEN LOCATION MAP</a></div>`;
}
