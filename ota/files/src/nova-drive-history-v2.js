...
async function b64DataUrl(url){
  ...
  if(!r.ok) throw new Error(`Drive History visual ${r.status}`);
...
function installStyle(){
  ...
  .nx-approved-history-v2{position:absolute!important;inset:0!important;z-index:68!important;overflow:hidden!important;background:#000!important}
...
function mount(ui){
  ...
  async function reload(){const state=await loadDriveTrackState();store=state?.store||{trips:[]};if(open)render();}
  function openHistory(next='today'){filter=FILTERS.has(next)?next:'today';selected=null;open=true;history.classList.add('is-active');history.setAttribute('aria-hidden','false');render();reload().catch(()=>{});}
  function closeHistory(){open=false;history.classList.remove('is-active');history.setAttribute('aria-hidden','true');}
  [['[data-dr-today]','today'],['[data-dr-week]','week'],['[data-dr-month]','month'],['[data-dr-count]','history']].forEach(([sel,f])=>{const card=drive.querySelector(sel)?.closest('.nxdr3-stat');if(card){card.style.cursor='pointer';card.style.touchAction='manipulation';card.addEventListener('click',()=>openHistory(f));}});
  [['today','today'],['week','week'],['month','month'],['history','history']].forEach(([cls,f])=>{const hit=document.createElement('button');hit.type='button';hit.className=`nx-history-entry-v2 ${cls}`;hit.style.cssText=`top:64.1%;height:8.0%;left:${cls==='today'?'3.8%':cls==='week'?'26.9%':cls==='month'?'50.0%':'73.1%'};width:${cls==='history'?'20.8%':'20.0%'}`;hit.setAttribute('aria-label',`Open Drive History ${f}`);hit.onclick=()=>openHistory(f);drive.appendChild(hit);});
