...
const meshFS=['#version 300 es','precision highp float;','uniform vec3 uColor;','uniform float uOpacity;','uniform float uEmission;','uniform float uPattern;','uniform float uTime;','in vec3 vNormal;','in vec3 vLocal;','out vec4 outColor;','void main(){vec3 n=normalize(vNormal);float l=.52+.48*max(0.,dot(n,normalize(vec3(.45,.78,.32))));float rim=pow(1.-max(0.,dot(n,vec3(0.,0.,1.))),2.0);float grain=.97+.03*sin((vLocal.x+vLocal.y*1.7+vLocal.z*.8+uTime*.15)*20.);vec3 base=uColor;if(uPattern>.5&&uPattern<1.5){float h=sin(vLocal.x*6.7+vLocal.z*10.3+sin(vLocal.y*5.2))*0.5+.5;float land=smoothstep(.62,.76,h+.18*sin(vLocal.z*15.0));base=mix(vec3(.07,.27,.82),vec3(.10,.47,.16),land);base=mix(base,vec3(.78,.88,.96),smoothstep(.76,.96,abs(vLocal.y)*.92)*.32);}else if(uPattern>=1.5&&uPattern<2.5){float bands=.5+.5*sin(vLocal.y*20.0+sin(vLocal.x*3.0+uTime*.05));base=mix(vec3(.62,.38,.2),vec3(.92,.72,.46),smoothstep(.28,.72,bands));}else if(uPattern>=2.5){float cells=.5+.5*sin(vLocal.x*19.0+uTime*.7)*sin(vLocal.z*23.0-uTime*.45);base=mix(vec3(.9,.22,.04),vec3(1.0,.76,.16),cells);}vec3 c=base*(l*grain)+base*(uEmission*(.5+.5*rim));outColor=vec4(c,uOpacity);}'].join('\n');
...
let W=1,H=1,dpr=1,lastFrame=performance.now(),fpsEMA=60,frameN=0;function resize(){W=Math.max(1,innerWidth);H=Math.max(1,innerHeight);dpr=Math.min(DPR_MAX,Math.max(1,devicePixelRatio||1));canvas.width=Math.floor(W*dpr);canvas.height=Math.floor(H*dpr);canvas.style.width=W+'px';canvas.style.height=H+'px';gl.viewport(0,0,canvas.width,canvas.height);}addEventListener('resize',resize,{passive:true});resize();
...
function makeGalaxyStarArray(seed,q){
 const count=Math.floor(3600*Math.max(.5,Math.min(1,q))),out=[];
 for(let i=0;i<count;i++){
  const u=rand(seed+'gsu',i),v=rand(seed+'gsv',i),w=rand(seed+'gsw',i),arm=(i%5)*TAU/5,rad=.25+Math.pow(v,.58)*6.1,ang=arm+rad*1.48+(u-.5)*.6;
  const y=(w-.5)*(.18+.55*(1-rad/6.4)),x=Math.cos(ang)*rad,z=Math.sin(ang)*rad,tc=rand(seed+'gst',i);
  const col=tc<.17?[1,.6,.35,1]:tc<.43?[1,.82,.52,1]:tc<.78?[.68,.84,1,1]:[.52,.72,1,1];
  out.push({p:[x,y,z],c:col,s:.35+rand(seed+'gss',i)*1.55});
 }
 return out;
}
function ensureGalaxyStars(seed,q){
 const k=String(seed)+'|'+Math.round(q*100);if(galaxyStarsKey===k)return;
 if(galaxyStarsBuf)delPoint(galaxyStarsBuf);galaxyStarsBuf=makePointBuffer(makeGalaxyStarArray(seed,q),gl.STATIC_DRAW);galaxyStarsKey=k;
}

function makeDustArray(seed,q){
 const count=Math.floor(900*Math.max(.5,Math.min(1,q))),out=[];
 for(let i=0;i<count;i++){const a=rand(seed+'dustA',i)*TAU,r=.8+Math.pow(rand(seed+'dustR',i),.62)*6.1,y=(rand(seed+'dustY',i)-.5)*(.16+.32*(1-r/7)),x=Math.cos(a)*r,z=Math.sin(a)*r,swirl=rand(seed+'dustS',i);out.push({p:[x,y,z],c:swirl<.45?[.42,.62,1,.13]:swirl<.8?[.68,.4,1,.11]:[.2,.42,.72,.08],s:.65+rand(seed+'dustSize',i)*1.6});}
 return out;
}
function ensureDust(seed,q){
 const k=String(seed)+'|'+Math.round(q*100);if(dustKey===k)return;
 if(dustBuf)delPoint(dustBuf);dustBuf=makePointBuffer(makeDustArray(seed,q),gl.STATIC_DRAW);dustKey=k;
}
...
let galaxyKey='',galaxyScene=null,starsKey='',starsBuf=null,galaxyStarsKey='',galaxyStarsBuf=null,catBuf=null,dustKey='',dustBuf=null,projected=[],webNetworkNodes=[];
...
function getScene(depth,seed,q,family){const sceneDepth=Math.max(0,Number(depth)||0),key=sceneDepth+'|'+String(family||0)+'|'+Number(seed).toFixed(5)+'|'+Math.round(q*100);if(starsKey!==key){if(starsBuf)delPoint(starsBuf);starsBuf=makePointBuffer(makePointArray(seed,q,sceneDepth),gl.STATIC_DRAW);starsKey=key;}
 if(sceneDepth>=2&&sceneDepth<=7)ensureGalaxy(seed);return sceneMode({depth:sceneDepth,family:Number(family)||0});}
...
function drawSolarScene(s,mat,t){
 ...
 const sun=model(0,0,0,.72,.72,.72,t*.045,0,0);gl.blendFunc(gl.SRC_ALPHA,gl.ONE);drawMesh(solarSphere,mat,model(0,0,0,.91,.91,.91,t*.02,0,0),[1,.3,.03],.055,2.0,3,t);glowBlob(mat,sun,[1,.53,.12],1,2.7,3,t);
 const planets=[
 ...
 planets.forEach((p,i)=>{
   ...
   if(p.name==='Earth'){drawMesh(solarSphere,mat,model(x,0,z,p.s*1.09,p.s*1.09,p.s*1.09,0,t*.04,0),[.18,.54,1],.10,.65,0,t);const ma=t*.62,mx=x+Math.cos(ma)*.34,mz=z+Math.sin(ma)*.34;drawMesh(solarSphere,mat,model(mx,0,mz,.055,.055,.055,0,t*.2,0),[.75,.78,.84],1,.18,0,t);}
...
function drawMilkyWayScene(s,mat,t){
 ensureGalaxy(s.seed);ensureGalaxyStars(s.seed,s.quality);
 gl.blendFunc(gl.SRC_ALPHA,gl.ONE_MINUS_SRC_ALPHA);gl.depthMask(false);
 ensureDust(s.seed,s.quality);drawPoints(dustBuf,mat,7);drawMesh(galaxyScene.disk,mat,model(0,0,0,5.9,.055,5.9,0,0,0),[.08,.24,.46],.48,.55);
 ...
 drawRealStarLayer(s,mat,18,.65);drawPoints(galaxyStarsBuf,mat,11);
...
function drawNebulaScene(s,mat,t){
 gl.blendFunc(gl.SRC_ALPHA,gl.ONE);gl.depthMask(false);ensureDust(s.seed,s.quality);drawPoints(dustBuf,mat,8);
