...
const TAU = Math.PI * 2;
const WORLDS = [
  {name:'THE FIRST FIELD',family:'COSMIC FIELD',type:'PROCEDURAL VISUALIZATION',scale:'COSMOLOGICAL CONTEXT',desc:'A bounded visual context around public-catalog layers. It is a navigation surface, not a complete map of the observable universe.'},
  {name:'COSMIC FILAMENTS',family:'LARGE-SCALE STRUCTURE',type:'DERIVED FROM SOURCE DATA',scale:'LARGE-SCALE STRUCTURE',desc:'A survey-inspired network for exploring redshift depth and galaxy distributions. Filaments are visualized from sampled records; they are not claimed as a complete observed cosmic web.'},
  {name:'SPIRAL MEMORY',family:'GALAXY',type:'PROCEDURAL VISUALIZATION',scale:'GALAXY SCALE',desc:'A deterministic spiral galaxy visualization. Arm geometry, dust and bulge are reconstructed for navigation and are not telescope surface imagery.'},
  {name:'ORBITAL GARDEN',family:'STELLAR REGION',type:'REAL CATALOG + PROCEDURAL',scale:'STELLAR NEIGHBORHOOD',desc:'Catalog-backed stars are placed over a deterministic stellar-region context. Point positions and measurements keep their source provenance.'},
  {name:'STAR SYSTEM',family:'STAR SYSTEM',type:'REAL CATALOG + PROCEDURAL',scale:'STAR SYSTEM',desc:'Host-star and exoplanet records can be inspected where the public archive reports them. Orbits are visual guides, not live ephemerides.'},
  ...
  {name:'NEON DISTRICT',family:'CITY LAYER',type:'PROCEDURAL VISUALIZATION',scale:'ILLUSTRATIVE MICRO-SCALE',desc:'A deliberately illustrative city-like continuation inside the exploration chain. It is not a real mapped city unless a separate source record is shown.'},
  ...
  {name:'CELLULAR SEA',family:'MICRO WORLD',type:'PROCEDURAL VISUALIZATION',scale:'MICRO SCALE',desc:'A bounded microscopic continuation where repeated structures expose another visual grammar.'},
  {name:'RECURSIVE VAULT',family:'FRACTAL WORLD',type:'PROCEDURAL VISUALIZATION',scale:'RECURSIVE SCALE',desc:'A finite recursive scene with a fixed computation budget. The recursion is visual, not a claim about physical reality.'},
...
function home(){reset();state.yaw=0;state.pitch=.16;updateHUD();emitEvent('HOME SCALE / MILKY WAY CONTEXT');}
...
function commitDepth(custom){const old={depth:state.depth,family:state.family,seed:state.seed};state.history.push(old);const n=custom||nextWorld();state.depth=n.depth;state.family=n.family;state.seed=n.seed;state.panX=state.panY=state.targetPanX=state.targetPanY=0;state.targetDistance=8.2;state.distance=state.targetDistance;state.portalArmed=false;state.transition=reducedMotion?.15:1;state.selectedId=null;updateHUD();saveDiscovery('New world: '+currentWorld().family,state.worldId);emitEvent('DESCENT COMPLETE / '+currentWorld().family);}
...
function updatePhysics(){const dt=1/60;state.distance+=(state.targetDistance-state.distance)*.12;state.panX+=(state.targetPanX-state.panX)*.12;state.panY+=(state.targetPanY-state.panY)*.12;if(state.transition>0)state.transition=Math.max(0,state.transition-(reducedMotion?.055:.028));if(state.distance<1.38 && state.portalArmed)queueDive();rebaseIfNeeded();}
...
function pointerUp(e){const p={x:e.clientX,y:e.clientY};const now=performance.now();const moved=down?Math.hypot(p.x-down.x,p.y-down.y):999;pointers.delete(e.pointerId);if(!pointers.size){dragging=false;pointerMove.lastX=null;pointerMove.lastY=null;}if(now-(lastTap?.t||0)<360&&Math.hypot(p.x-(lastTap?.x||9999),p.y-(lastTap?.y||9999))<30){queueDive();lastTap=null;}else if(moved<13){window.NexusNovaInfiniteLabRenderer?.handleTap?.(p.x,p.y);}else lastTap={x:p.x,y:p.y,t:now};}
...
window.NexusNovaInfiniteLab={
  ...
  getDebug:()=>({webgl2:true,primaryRenderer:'WebGL2',boundedDpr:state.boundedDpr,lod:state.lodEnabled,culling:state.cullingEnabled,cache:{size:window.NexusNovaInfiniteLabCatalog?.getDebug?.().cacheSize||0,max:state.cacheMax},staleProtection:state.staleProtection,originRebaseCount:state.originRebaseCount,queriesInFlight:state.queriesInFlight,sourceAdapters:window.NexusNovaInfiniteLabCatalog?.getDebug?.().sources||[],worldDepth:state.depth,worldId:state.worldId}),
