...
(()=>{
  ...
  if(nav){
    ...
    if(!nav.querySelector(`a[href="${popularHref}"]`)){
      ...
      const guide=nav.querySelector(`a[href="${base}guides.html"]`);
      const link=document.createElement('a');
      link.href=popularHref;
      link.textContent='Popular Tools';
      if(daily) daily.insertAdjacentElement('afterend',link);
      else if(guide) guide.insertAdjacentElement('beforebegin',link);
      else nav.appendChild(link);
    }

    const accountHref=`${base}register.html`;
    if(!nav.querySelector(`a[href="${accountHref}"]`)){
      const link=document.createElement('a');
  ...
  if(path==='index.html'&&!document.querySelector('[data-popular-banner]')){
    const firstSection=document.querySelector('main .section');
    if(firstSection){
      const wrap=document.createElement('section');
      wrap.className='section';
      wrap.dataset.popularBanner='';
      wrap.innerHTML='<div class="container"><div class="cta"><div><span class="kicker">Popular right now</span><h2>Images, PDF, resumes and AI productivity.</h2><p>Research-led browser tools for common digital tasks, built without fake results.</p></div><a class="btn btn-primary" href="popular-tools.html">Open Popular Tools →</a></div></div>';
      firstSection.insertAdjacentElement('beforebegin',wrap);
    }
  }
  ...
  if(path==='tools.html'&&!document.querySelector('[data-popular-tools-notice]')){
    ...
    if(search){
      ...
      note.innerHTML='<strong>Need image, PDF, resume or AI prompt tools?</strong> <a href="popular-tools.html" style="color:var(--accent);font-weight:800">Open Popular Tools →</a>';
